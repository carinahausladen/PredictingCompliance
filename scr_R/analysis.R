...
mean(df$player.age)
paste('share females:',
      round(table(df$player.gender)[2]/sum(table(df$player.gender)), digits = 2))
...
# Chat data cleaning ####
docs <- Corpus(VectorSource(df$player.exp_demand_open))
...
table(df$player.exp_demand_chat)
dd<-as.data.frame(levels(df$player.exp_demand_chat_open))
