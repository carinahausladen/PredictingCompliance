...
def str_grp_splt(df,
    ...
    Args:
        ...
        y_col_name (str, optional): Define label of which distribution in train and test shuold be similar to whole df.
    ...
    # for two labels
    train_grp_medium = []
    test_grp_medium = []  # open empty lists: if they are not needed they are just empty
    if len(lbl_cnts.index) > 2:
        ...
        cnt_grp = cnt_grp.loc[~cnt_grp.grp_col_name.isin(
            train_grp_mnrty + test_grp_mnrty)]  # exclude groups which are already taken
        ...
        test_grp_medium = list(set(cnt_grp.index.get_level_values(level=0)) - set(train_grp_mnrty))
